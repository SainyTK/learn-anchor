...
export class BankrunProvider2 extends BankrunProvider {
	constructor(context: ProgramTestContext, connection: BankrunConnection) {
		super(context);
		this.connection = connection.toConnection();
	}
	...
	async sendTransaction(
		...
		return await (this.connection as unknown as BankrunConnection).sendTransaction(tx);
	}

	async sendAndConfirm(tx: Transaction | VersionedTransaction, signers?: Signer[] | undefined, opts?: ConfirmOptions | undefined): Promise<string> {
		return this.sendTransaction(tx, signers as Keypair[])
	}
}
...
export class BankrunContextWrapper {
	...
	public readonly provider: BankrunProvider2;
	...
	constructor(context: ProgramTestContext, verifySignatures = true) {
		...
		this.provider = new BankrunProvider2(context, this.connection);
	}
...
export class BankrunConnection {
	...
	public rpcEndpoint = "http://localhost";
